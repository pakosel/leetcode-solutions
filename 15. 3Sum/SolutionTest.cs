...
namespace ThreeSum
{
    ...
    public class SolutionTest
    {
        ...
        public void Test_Better(string input, string expected)
        {
            ...
            List<IList<int>> expectedList = new List<IList<int>>();
            var arrExpexted = expected.TrimStart('[').TrimEnd(']').Split("],[");
            foreach(var strArr in arrExpexted)
            {
                var innerArr = strArr.Split(',');
                var innerNums = new int[innerArr.Length];
                for(int i=0; i<innerArr.Length; i++)
                    innerNums[i] = int.Parse(innerArr[i]);
                expectedList.Add(innerNums);
            }

            CollectionAssert.AreEquivalent(ret, expectedList);
        }
    }
}